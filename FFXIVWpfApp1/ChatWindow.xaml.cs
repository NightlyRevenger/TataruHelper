...
namespace FFXIITataruHelper
{
     ...
     public partial class ChatWindow : Window
     {
          ...
          void InitTataruModel()
          {
               ...
               _TataruModel.ChatProcessor.TranslationArrived += OnPlaySound;
          }
          ...
          private Dictionary<string, Gender> _genderMap = new Dictionary<string, Gender>()
          {
               { "Канчана", Gender.Female },
               { "Emeloth",Gender.Female }
          };

          private async Task OnPlaySound(TranslationArrivedEventArgs e)
          {
               await this.UIThreadAsync(async () =>
               {

                    string text = e.Text.Split(':').Last();
                    string name = e.Text.Split(':').First();
                    if (!_genderMap.TryGetValue(name, out var gender)) gender = Gender.Male;

                    string genderString = gender == Gender.Male ? "male" : "female";
                    var requestUri = "https://" + $"code.responsivevoice.org/getvoice.php?t={text}&tl=ru&sv=g3&vn=&pitch=0.5&rate=0.5&vol=1&gender={genderString}";

                    var s = Guid.NewGuid() + ".mp3";
                    using (HttpClient client = new HttpClient())
                    using (var stream = await client.GetStreamAsync(requestUri).ConfigureAwait(false))
                    using (var fileStream = File.Create(s))
                    {
                         await stream.CopyToAsync(fileStream);
                    }

                    var source = new Uri(s, UriKind.Relative);
          ...
          {
               string filePath = ((MediaElement) sender).Source.ToString();
